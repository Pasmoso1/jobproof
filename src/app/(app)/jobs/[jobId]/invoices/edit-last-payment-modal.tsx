...
export function EditLastPaymentModal({
  ...
  const formKey = `${payment.id}-${payment.amount}-${payment.paid_on}-${payment.payment_method}-${payment.note ?? ""}`;
    ...
    >
      ...
      >
        <EditLastPaymentForm
          ...
          invoiceId={invoiceId}
          payment={payment}
          maxTotalForLastPayment={maxTotalForLastPayment}
          onClose={onClose}
        />
      </div>
    </div>
